...
## Flask sessions to fix unauthorized logging in when passing in production data

When the development goes from local to production, despite logging in with correct credentials, the system doesn't let the authentication happen despite the Network tab is sending a 200 status from the backend. To fix this, we'll store the login data in a session. This process will be done in the backend.

First, we'll install flask sessions: ``pip install flask-session``.

Then, we'll pass in this to the backend: 
...
> Secret Keys are generated like 
> import secrets
> secret_key = secrets.token_hex(16)
# example output, secret_key = 000d88cd9d90036ebdd237eb6b0db000
> app.config['SECRET_KEY'] = secret_key``

Now we'll modify the routes to store the login info in a session in the login route:
...
We also need to modify the frontend. When making requests from React to your Flask backend, you need to ensure that credentials (such as cookies) are included. This allows the session data to be sent along with each request, enabling the backend to identify the user.
